...
uint32_t QuicServer::DecodeHexBuffer(
...
#pragma endregion
    ...
    if (QUIC_FAILED(Status = MsQuic->ConfigurationOpen(Registration, &Alpn, 1, &Settings, sizeof(Settings), this, &Configuration))) {
...
QUIC_STATUS QUIC_API QuicServer::StaticServerConnectionCallback(
    ...
    return reinterpret_cast<QuicServer*>(Context)->ServerConnectionCallback(Connection, Context, Event);
}

_IRQL_requires_max_(DISPATCH_LEVEL) _Function_class_(QUIC_STREAM_CALLBACK) QUIC_STATUS QUIC_API QuicServer::ClientStreamCallback(
    _In_ HQUIC Stream,
    _In_opt_ void* Context,
    ...
    case QUIC_STREAM_EVENT_RECEIVE:
    {
        ...
        std::vector<uint8_t> receivedData(data, data + Event->RECEIVE.TotalBufferLength);

        uint8_t endpoint = receivedData[0];
        uint8_t identifier = receivedData[1];
        std::vector<uint8_t> message(receivedData.begin() + 2, receivedData.end());

        this->inStreams[Stream].insert(this->inStreams[Stream].end(), receivedData.begin(), receivedData.end());

        // auto x_function = Nexus::endPoints.find(endpoint);
        // auto x_object = Nexus::deserializers.find(identifier);

        //try {
        //    if (x_function != Nexus::endPoints.end() && x_object != Nexus::deserializers.end()) {
        //        nexus_base deserializedObject = x_object->second.deserializer(message);
        //        
        //        x_function->second(&deserializedObject, &endpoint);
...
QUIC_STATUS QUIC_API QuicServer::StaticClientStreamCallback(
    ...
    return reinterpret_cast<QuicServer*>(Context)->ClientStreamCallback(Stream, Context, Event);
}

_IRQL_requires_max_(DISPATCH_LEVEL)
_Function_class_(QUIC_CONNECTION_CALLBACK)
QUIC_STATUS
QUIC_API QuicServer::ServerConnectionCallback(
    _In_ HQUIC Connection,
    _In_opt_ void* Context,
    _Inout_ QUIC_CONNECTION_EVENT* Event
) {
    ...
    case QUIC_CONNECTION_EVENT_PEER_STREAM_STARTED: {
        ...
        if (this->inStreams.find(Event->PEER_STREAM_STARTED.Stream) == this->inStreams.end()) {
            this->inStreams[Event->PEER_STREAM_STARTED.Stream] = std::vector<uint8_t>();
        }
        HQUIC Stream = Event->PEER_STREAM_STARTED.Stream;
        MsQuic->SetCallbackHandler(Stream, (void*)QuicServer::StaticClientStreamCallback, this);
        break;
    }
...
QUIC_STATUS QUIC_API QuicServer::ServerListenerCallback(_In_ HQUIC Listener,
    _In_opt_ void* Context,
    ...
    case QUIC_LISTENER_EVENT_NEW_CONNECTION:
        MsQuic->SetCallbackHandler(Event->NEW_CONNECTION.Connection, (void*)QuicServer::StaticServerConnectionCallback, this);
...
QUIC_STATUS QUIC_API QuicServer::StaticServerListenerCallback(
    ...
    return reinterpret_cast<QuicServer*>(Context)->ServerListenerCallback(Listener, Context, Event);
}
...
void QuicServer::Start() {
    ...
    if (!this->isRunning.load()) {
        ...
        this->serverThread = std::thread([this]() {
                ...
                std::cout << "uwu" << "\n";
                if (QUIC_FAILED(Status = MsQuic->ListenerOpen(Registration, QuicServer::StaticServerListenerCallback, this, &Listener))) {
    ...
    if (this->isRunning.load()) {
        ...
        if (this->serverThread.joinable()) {
            ...
            if (!this->serverThread.joinable()) {
                ...
                if (Listener != NULL) {
                    std::cout << "Server thread has terminated successfullyX." << std::endl;
                    MsQuic->ListenerClose(Listener);
                }
                std::cout << "Server thread has terminated successfully." << std::endl;
            }
                ...
                // You might consider additional handling here, such as forcefully terminating the thread
            }
        }
        else {
...
bool QuicServer::getIsRunning() {
    return this->isRunning.load();
}
QuicServer::QuicServer(const char* Host, const uint16_t UdpPort, const char* cert, const char* key) {
    this->Host = (char*)Host;
